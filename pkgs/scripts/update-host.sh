...
if [[ ! ${args[remote-eval]:-} ]] ; then
    ...
    drvPath=$( @{pkgs.push-flake!getExe} "$hostname" . )#nixosConfigurations."$hostname".config.system.build.toplevel || exit
fi
...
} ; declare -f remote ) ; remote $( printf ' %q' "$drvPath" "$predicate" "${argv[@]:2}" )" || exit
