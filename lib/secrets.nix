        ...
        ${appName} = lib.fun.writeTextFiles pkgs "agenix-wrapped" {
            ...
            "bin/${appName}" = ''
                ...
                if [[ $identity && ''${1:-} && ''${2:-} ]] ; then
                    ...
                    genPW= ; if [[ $1 == -p || $1 == --genkey-password ]] ; then genPW=1 ; fi
                    edit= ; if [[ $1 == -e || $1 == --edit || $genSSH || $genWG || $genPW ]] ; then edit=true ; fi
                    ...
                    if [[ $edit && ! -s $2 ]] ; then
                        ...
                        if [[ ! $existed ]] ; then git stage "$2" ; exec nix run .#${appName} -- "$@" ; fi # re-eval with new file
                    fi
